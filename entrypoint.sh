...
if [ ! -f ${SLAPD_CONF_DIR}/${INSTALL_CONTROL_FILE} ]; then
   ...
   # if var SLAPD_ROOTDN empty, then set it to SLAPD_ROOTDN=cn=admin,${SLAPD_SUFFIX}. 
   # Vars SLAPD_ROOTDN and SLAPD_ROOTDN are defined in .env
   [[ -z "${SLAPD_ROOTDN}" ]] && SLAPD_ROOTDN=cn=admin,${SLAPD_SUFFIX}

   # if var SLAPD_ROOTPW is not empty
   if [ ! -z "${SLAPD_ROOTPW}" ]; then
      echo "INFO: Var \"SLAPD_ROOTPW\"is set. "
      echo -n "Setting hashed RootDN password...               "
      # vars: PASSWORD_LOAD_MODULE, SLAPD_PASSWORD_HASH and SLAPD_ROOTPW are defined in .env
      SLAPD_ROOTPW_HASH=$(slappasswd -o module-load=${PASSWORD_LOAD_MODULE} -h {${SLAPD_PASSWORD_HASH}} -s "${SLAPD_ROOTPW}")
      echo "[ DONE ]"
   # if var SLAPD_ROOTPW_HASH is empty and file /run/secrets/$SLAPD_ROOTPW_SECRET exist and has a size greater than zero (-s)  
   ...
   # if directory /etc/ssl/openldap exist
   ...
   # if data.mdb does not exist (man test)
