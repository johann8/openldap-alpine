...
if [ ! -f ${SLAPD_CONF_DIR}/${INSTALL_CONTROL_FILE} ]; then
    ...
    if [ ! -z "$SLAPD_ROOTPW" ]; then
        ...
        SLAPD_ROOTPW_HASH=$(slappasswd -o module-load=${PASSWORD_LOAD_MODULE} -h ${PASSWORD_HASH} -s "$SLAPD_ROOTPW")
        echo "[ DONE ]"
    elif [[ -z "$SLAPD_ROOTPW_HASH" && -s /run/secrets/$SLAPD_ROOTPW_SECRET ]]; then
        echo -n "Setting hashed RootDN password...                "
        SLAPD_ROOTPW_HASH=$(slappasswd -o module-load=${PASSWORD_LOAD_MODULE} -h ${PASSWORD_HASH} -s "$(cat /run/secrets/$SLAPD_ROOTPW_SECRET)")
         echo "[ DONE ]"
    fi
